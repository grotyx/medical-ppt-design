...
>   Use level structure: lead/primary lines **28pt** with **●** bullets — the dot a **fixed 28pt** (not a
>   % of each line, so every dot is identical) — and **indent smaller sub-detail one level** (level 2 —
>   `–` marker fixed 24pt, deeper indent, **24pt** gray) for a clean hierarchy.
